...
;-------------------------------------------------------
f1=addfile("${dir1}/${s}/olr.ano.19900101-20091231.nc","r")
TIME=f1->time
time=TIME
grid_yt=f1->grid_yt
grid_xt=f1->grid_xt
...
f2=addfile("${dir1}/${s}/ucomp.ano.19900101-20091231.nc","r")
...
;------------------------------------------------------------
 ...
 mjo_ts_index_ON@long_name = "MJO_${s} PC INDEX"
 mjo_ts_index_ON@info      = "sqrt of (PC1^2 + PC2^2)"

 ON_PC1=eof_ts_ON(0,:)
 ON_PC1!0="time"
 ON_PC1&time=time
 ON_PC1@long_name= "PC1 of ${s} experiment"
 ON_PC1@info      = "PC1/stddev(PC1)"

 ON_PC2=eof_ts_ON(1,:)
 ON_PC2!0="time"
 ON_PC2&time=time
 ON_PC2@long_name= "PC2 of ${s} experiment"
 ON_PC2@info      = "PC2/stddev(PC2)"

 system("rm -f ${dir3}/MJO_${s}_Index_PC1_2_19900101-20091231.nc")
 out=addfile( "${dir3}/MJO_${s}_Index_PC1_2_19900101-20091231.nc","c")
...
;------------------------------------------------------------
 ...
 ;day@long_name = "lag (day)"
  ...
  wks = gsn_open_wks("png","${dir2}/MJO_${s}_30-60_bandpass_19900101-20091231")
  plot = new(3,graphic)
;************************************************
...
;  rtsP@gsnMaximize           = True                ; large format
  rtsP@gsnPanelMainString     = "Multivariate EOF (${s}): 15S-15N: "+yrStrt+"-"+yrLast
  do n=0,neof-1
...
;-----------------------------------------
  ...
  rtsP@gsnPanelMainString    = "Cross Correlation: Multivariate EOF (${s}): 15S-15N: " \
                   +  yrStrt+"-"+yrLast
...
;************************************************
  ...
  rtsP@gsnPanelMainString   = "MJO Index(${s}): square root of (PC1^2+ PC2^2) : 15S-15N: "+yrStrt+"-"+yrLast
  gsn_panel(wks,plot(0),(/1,1/),rtsP)     ; now draw as one plot
