...
for months in "6_8" "12_2" ;do
   if [ $months = "12_2" ];then
    season="DJF"
    A="res@cnMaxLevelValF         =      60"
    B="res@cnMinLevelValF         =      -60"
    C="res@cnLevelSpacingF        =       10"
   else
    season="JJA"
    A="res@cnMaxLevelValF         =      60"
    B="res@cnMinLevelValF         =      -60"
...
f=addfile(dir1+"OLR_map_${SSS}_${months}_MJO_Phase_8_period_${N}.nc","r")
...
plot=new(8,graphic)
wks=gsn_open_wks("png",dir2+"MJO_${SSS}_${months}_Phase_Map_ON_period_${N}")
res                        =   True
res@gsnFrame               =       False
res@gsnDraw                =       False
;res@vpHeightF=0.2
...
resP@gsnPanelMainString  = "${M}; ${SSS} ; ${season} "
