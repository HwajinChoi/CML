...
HGT_dur=new((/5,3/),"float")
HGT_dur(0,:)=0
HGT_dur(1,:)=(/71,86,23/) ;June
HGT_dur(2,:)=(/0,0,65/) ; Jul
HGT_dur(3,:)=0 ;Aug
HGT_dur(4,:)=(/9,0,0/) ; Sep
HGT_dur!0="month"
HGT_dur!1="case"
...
wks = gsn_open_wks("png",dir2+"Duration_moving_2016_2018_2020_clim_hgt_${h}")
plot=new(3,"graphic")
plot1=new(3,"graphic")
 res   = True
 ...
 plot(0) = gsn_csm_xy (wks,fspan(0.8,4.8,5),HGT_dur(:,0),res)
 res@gsnCenterString=""
 res@xyLineColors       = (/"Blue"/)
 res@gsnXYBarChartColors   =(/"blue"/)
 plot(1) = gsn_csm_xy (wks,fspan(1.0,5.0,5),HGT_dur(:,1),res)
 res@xyLineColors       = (/"Green"/)
 res@gsnXYBarChartColors   =(/"green"/)
 plot(2) = gsn_csm_xy (wks,fspan(1.2,5.2,5),HGT_dur(:,2),res)
