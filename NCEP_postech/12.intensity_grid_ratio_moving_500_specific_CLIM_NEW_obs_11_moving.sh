...
printVarSummary(tas_mv_11)
printVarSummary(tas_mv_11_k)
;exit
...
; TAS_mv_11_cases -> 3(case) * 112(time)

;----- FOR CORR -----------------------
hgt_mv_11_7_9=hgt_mv_11(:,31:142)
TAS_mv_11_cases_7_9=TAS_mv_11_cases

;-- It is correlations of fully data between HGT and TAS
corr=escorc_n(hgt_mv_11_7_9,TAS_mv_11_cases_7_9,1,1)
;-----------------------------------------
...
;---------------------------------------------------------------------------
do K=0,2
 wks = gsn_open_wks("png",dir2+"6_9_NEW_OBS_fill_"+C+"_Intensity_count_moving_"+YEAR(K)+"_clim_hgt_${h}_CLIM_"+per)
...
;---Set a title on right Y axis.
 ...
 xy_plot= gsn_csm_xy (wks,y2,TAS_mv_11_cases_7_9(K,:),xyres)
