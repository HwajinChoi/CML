...
OLR2=where(OLR .gt. 0,OLR,OLR@_FillValue)
copy_VarMeta(OLR,OLR2)
...
do F=0,31
 OLR3(0,F,:)=where(.not.ismissing(OLR2(0,F,:)),F+0.85,0) ; indo
 OLR3(1,F,:)=where(.not.ismissing(OLR2(1,F,:)),F+1.15,0) ; china
end do
OLR3@_FillValue=0
...
yyyy=ispan(37,148,1)
;---------------------------------------------------------------------------
wks = gsn_open_wks("png",dir2+"For_COUNT_moving_Indo_China_91-22_hgt_${h}_"+per)
plot=new(2,"graphic")
plot1=new(2,"graphic")
plot2=new(2,"graphic")
 res   = True
...
pres@gsnPanelMainString        =   "NPH & NW India, S. China sea"
