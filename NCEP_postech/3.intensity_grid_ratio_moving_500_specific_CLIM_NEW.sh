...
wks = gsn_open_wks("png",dir2+"NEW_Intensity_count_moving_2016_2018_2020_clim_hgt_${h}_CLIM_"+per)
plot=new(4,"graphic")
 res   = True
...
; res@tmYLPrecision=1
 ...
 plot(0) = gsn_csm_xy (wks,fspan(5.7,147.7,143),hgt_mv_area(0,:),res)
 res@gsnCenterString=""
 res@xyLineColor       = (/"Blue"/)
 res@gsnXYBarChartColor   =(/"Blue"/)
 plot(1) = gsn_csm_xy (wks,fspan(5.9,147.9,143),hgt_mv_area(1,:),res)
 res@xyLineColor       = (/"Green"/)
 res@gsnXYBarChartColor   =(/"Green"/)
 plot(2) = gsn_csm_xy (wks,fspan(6.1,148.1,143),hgt_mv_area(2,:),res)
 res@xyLineColor       = (/"black"/)
 res@gsnXYBarChartColor   =(/"black"/)
 plot(3) = gsn_csm_xy (wks,fspan(6.3,148.3,143),hgt_mv_area_CLIM,res)
