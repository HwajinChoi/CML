...
ListSetType(f,"join")
hgt_1=f[:]->hgt
hgt22=hgt_1(:,:,0,{43:31},{117:137}); year x time x level x lat x lon (117-137°E, 31-43°N)
hgt=dim_avg_n_Wrap(hgt22,0)

hgt_mask=where(hgt .ge. 5850, 1,0) ; for 500 hpa
;hgt_mask=where(hgt .ge. 12480, 1,0) ; for 200hpa
copy_VarMeta(hgt,hgt_mask)

test=dim_sum_n_Wrap(hgt_mask,1)
hgt_count=dim_sum_n_Wrap(test,1)
...
hgt_mv_11=new((/tp/),"float"); Target
imsi=new((/p/),"float"); Chunck

do k=0,tp-1
  imsi(:)=hgt_count2(k:k+p-1)
  hgt_mv_11(k)=dim_avg_n_Wrap(imsi,0)
