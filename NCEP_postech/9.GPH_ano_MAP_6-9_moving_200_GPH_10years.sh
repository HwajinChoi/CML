...
plot2(2)=gsn_csm_contour(wks,hgt_8,res4)
 ...
 gsn_text_ndc(wks,"H200",0.052,0.52,txres)
 gsn_text_ndc(wks,"(m)",0.94,0.38,txres)
 
 txres@txFontHeightF = 0.02                     ; text font height
 ;gsn_text_ndc(wks,"North Pacific high (200hpa)",0.55,0.87,txres)

 txres@txFontHeightF = 0.025                     ; text font height
  dum = new(3,graphic)
  dum(0) = gsn_add_text(wks,plot(0),"12480",130,23,txres)
  dum(1) = gsn_add_text(wks,plot(1),"12480",110,30,txres)
  dum(2) = gsn_add_text(wks,plot(2),"12480",160,35,txres)

 txres@txFontColor="green"
  dum1 = new(3,graphic)
  dum1(0) = gsn_add_text(wks,plot(0),"12480",150,31,txres)
  dum1(1) = gsn_add_text(wks,plot(1),"12480",150,35,txres)
  dum1(2) = gsn_add_text(wks,plot(2),"12480",110,41,txres)
