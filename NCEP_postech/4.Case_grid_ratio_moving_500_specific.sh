...
hgt_mv_1=where( .not.ismissing(hgt_mv_area(0,:)), 1, hgt_mv_area@_FillValue)
hgt_mv_2=where( .not.ismissing(hgt_mv_area(1,:)), 2, hgt_mv_area@_FillValue)
hgt_mv_3=where( .not.ismissing(hgt_mv_area(2,:)), 3, hgt_mv_area@_FillValue)
...
wks = gsn_open_wks("png",dir2+"For_COUNT_moving_2016_2018_2020_clim_hgt_${h}_"+per)
plot=new(3,"graphic")
plot1=new(3,"graphic")
 res   = True
 ...
 res@tmYLValues =(/0,1,2,3,4/)
 res@tmYLLabels = (/"","2016","2018","2020",""/)
 res@tmYLPrecision=1
 ...
 res@xyLineThicknesses    = (/10,10,10/)
 ...
 res@xyLineColor       = (/"red"/)
 plot(0) = gsn_csm_xy (wks,y5,HGT(0,:),res)
 res@gsnCenterString=""
 res@xyLineColor       = (/"Blue"/)
 plot(1) = gsn_csm_xy (wks,y5,HGT(1,:),res)
 res@xyLineColor       = (/"Green"/)
 plot(2) = gsn_csm_xy (wks,y5,HGT(2,:),res)
