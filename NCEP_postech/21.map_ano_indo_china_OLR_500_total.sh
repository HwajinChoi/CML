...
;---------------------------------
hh=addfile("${dir4}/OLR_11moving_1991-2020_June_Sep.nc","r")
olr_clim=hh->orl ; time x lat x lon

C=5850
dir2="/media/cmlws/Data2/hjc/NCEP/image/"
;-------------------------------------------------------------------
h=addfile("${dir3}/OLR_MAP_11moving_2016_2018_2020_June_Sep.nc","r")
olr_case=h->orl ; case x time x lat x lon 
di=dimsizes(olr_case)
olr=new((/di(0),di(1),di(2),di(3)/),"float")

do kkk=0,2
 olr(kkk,:,:,:)=olr_case(kkk,:,:,:)-olr_clim
end do
copy_VarMeta(olr_case,olr)
;-------------------------------------------------------------------
...
;exit
;---------------------------------------------------------------------------
 wks = gsn_open_wks("png",dir2+"MAP_OLR_ano_NPH_criteria_specific_years_${h}_"+per)
...
pres@gsnPanelMainString        =   "NPH & OLR ano"
