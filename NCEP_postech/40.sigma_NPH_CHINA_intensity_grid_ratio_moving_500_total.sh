...
for h in "500" ;do
dir1=/media/cmlws/Data2/hjc/NCEP/data/hgt_may_sep/${h}/specific2
dir2=/media/cmlws/Data2/hjc/NCEP/data/hgt_may_sep/${h}/for_clim
dir3=/media/cmlws/Data2/hjc/NCEP/data/obs/OLR/spec_2_6_9
...
;-------------------------------------------------------------------
h=addfile("${dir3}/OLR_indo_11moving_1995_2006_2010_2012_June_Sep.nc","r")
olr_indo=h->orl
olr_indo=olr_indo*(-1)
h2=addfile("${dir3}/OLR_china_11moving_1995_2006_2010_2012_June_Sep.nc","r")
olr_china=h2->orl
...
olr_indo_ano=new((/4,112/),"float")
olr_china_ano=new((/4,112/),"float")
...
hgt_mv_11=new((/4,tp/),"float"); Target
...
;;;; *hgt_mv_11*(4 case), hgt_mv_11_CLIM ;;;;;; 
...
;1995_2006_2010_2012
YEAR=(/"1995","2006","2010","2012"/)
COLOR=(/"BurlyWood","darkorchid","deepskyblue","LightSalmon"/)
;---------------------------------------------------------------------------
do K=0,3
 wks = gsn_open_wks("png",dir2+"SIG_6_9_ratio_total_Intensity_china_OLR_"+YEAR(K)+"_clim_hgt_${h}_"+per)
...
; res@tmYLPrecision=1
 ...
 res@tmYROn              = False
 res@gsnLeftString=""
 res@gsnCenterString="North Pacific high & OLR ("+YEAR(K)+")" 

  ;---XY curve resources
  xyres@tmYLMode="Manual"
  xyres@tmYLTickSpacingF=10
  xyres@tmYLMinorPerMajor=3
  xyres@xyLineThicknesses    = (/5.0/)               ; default is 1.0
  xyres@xyDashPatterns=(/0/)
  xyres@xyLineColors         = (/"purple"/)
...
;---Set a title on right Y axis.
 ...
 textres@lgLabels = (/"S. China Sea"/)  ; legend labels (required)
 bar_plot = simple_legend(wks,bar_plot,gres,lineres,textres)

 xy_plot= gsn_csm_xy (wks,y2,OLR(1,K,:),xyres)
